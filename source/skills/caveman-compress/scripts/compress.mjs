...
const INDENTED_TAB_PATTERN = /^\t/;
const INDENTED_BLOCK_PATTERN = /^ {4}/;
...
function compressText(text) {
	...
	for (const line of lines) {
		...
		if (
			inFence ||
			INDENTED_TAB_PATTERN.test(line) ||
			INDENTED_BLOCK_PATTERN.test(line)
		) {
			out.push(line);
			continue;
		}
		...
		if (
			FRONTMATTER_BOUNDARY_PATTERN.test(trimmed) ||
			FRONTMATTER_FIELD_PATTERN.test(trimmed)
		) {
			out.push(line);
			continue;
		}
		...
		const leading = line.match(LIST_PREFIX_PATTERN)?.[0] ?? "";
