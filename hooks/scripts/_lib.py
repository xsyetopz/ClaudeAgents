...
AI_PROSE_SLOP = [
    ...
    re.compile(
        r"\b(?:elevate|streamline|resonate|vibrant|bustling|tapestry|testament|paramount|imperative|indispensable|pivotal)\b",
        re.IGNORECASE,
    ),
...
WORKAROUND_HARD = [
    ...
    re.compile(
        r"(?:we|I)\s+(?:could|can)\s+(?:also|instead|alternatively)\s+(?:just\s+)?(?:use|do|make|change|switch|create|add)",
        re.IGNORECASE,
    ),
    re.compile(
        r"(?:make|change|switch)\s+\w+\s+(?:to\s+)?(?:public|private|protected|pub\b|pub\(crate\))", re.IGNORECASE
    ),
...
def allow(reason: str = "", event: str = "PreToolUse") -> NoReturn:
    result = {"hookSpecificOutput": {"hookEventName": event, "permissionDecision": "allow"}}
    if reason:
...
def audit_log(
    event: str, hook: str, action: str, reason: str = "", tool: str = "", extra: dict[str, Any] | None = None
) -> None:
