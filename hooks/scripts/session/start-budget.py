...
def file_line_count_exceeds_limit(base_dir: str, rel_path: str, limit: int) -> str | None:
    ...
    if full_path.exists():
        ...
        if lines > limit:
            return (
                f"[budget] {rel_path}: {lines} lines (target {limit}). Compact this file to reduce per-turn token cost."
            )
    return None


def collect_line_budget_warnings(base_dir: str) -> list[str]:
    warnings = [msg for rel_path, limit in TARGETS if (msg := file_line_count_exceeds_limit(base_dir, rel_path, limit))]
    return warnings
