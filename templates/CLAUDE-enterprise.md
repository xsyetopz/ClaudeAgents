...
- **Medium stakes** (data structure choice, API shape, dependencies, public naming, pattern deviation): treat as high stakes — present analysis + recommendation, wait for explicit approval
...
- No placeholder code: no stub markers, "in a real implementation", or incomplete function bodies
...
- All file modifications require prior plan approval
- Use the `audit_log` MCP tool to record decisions before acting
- Security findings from any agent block the workflow until resolved
- HTTP hooks forward all tool calls to the enterprise DLP server (CCA_HTTP_HOOK_URL)
- Never log, print, or store credentials, PII, or internal identifiers
...
- Start a fresh session at 300k tokens if reasoning quality matters
