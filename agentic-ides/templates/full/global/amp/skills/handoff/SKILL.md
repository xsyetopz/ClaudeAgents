...
Write to `.agents/session-handoff.md` in the project root using this format:
...
Check that `.agents/session-handoff.md` is covered by `.gitignore`. If not, add it:

```bash
echo ".agents/session-handoff.md" >> .gitignore
```
