# Where v3 Succeeded vs Fell Short

## V3 succeeded

### 1. It generated real artifacts

V3 generated provider-specific artifacts instead of asking users to paste a wall of instructions. It produced Claude agents/skills/hooks/templates, Codex agents/config/plugin skills/wrappers/hooks, OpenCode agents/skills/commands/plugin guardrails, and Copilot surfaces.
...
V3 generated into provider folders in-repo and tests inspected those outputs. This worked, but made it easy to blur source vs generated output.
...
Codex `implement` route had strong completion and anti-prototype language. OpenCode `openagents-implement` was much shorter. This is not automatically wrong, but OAL must intentionally decide provider-specific depth rather than accidentally under-rendering a provider.
...
V3 model tables included disallowed current OAL models such as `gpt-5.4`, `gpt-5.2`, and Claude Opus 4.6 variants. It also used `xhigh` too freely for Plus.
...
V3 install logic handled many surfaces, wrappers, prompts, toggles, plans, RTK, Caveman, Context7, Playwright, and provider-specific paths. It was useful, but hard to reason about as one long CLI.
...
V3 tested generated outputs and some install behavior, but a reboot should require fixture deploy/uninstall and runtime hook execution as acceptance conditions.
