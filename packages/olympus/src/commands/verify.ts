...
import { getOlympusCatalog, validateOlympusCatalog } from "../catalog";
...
import { hasLockDigestMismatch, readLock, writeLock } from "../lock";
...
export async function runVerify(json: boolean): Promise<ExitCode> {
	const report = await runAcceptanceSimulation();
	process.stdout.write(json ? asJson(report) : formatVerify(report));
	return report.ok ? 0 : 1;
}

async function runAcceptanceSimulation(): Promise<VerifyReport> {
