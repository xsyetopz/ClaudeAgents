...
function usage(): never {
	console.log(
		"Usage: oal accept|roadmap-evidence|check|preview [--provider all|codex|claude|opencode] [--path artifact] [--content]|render|generate [--provider all|codex|claude|opencode] [--out dir]|deploy --target dir [--scope project] [--provider all|codex|claude|opencode] [--dry-run]|uninstall --target dir --scope project --provider <provider>|toolchain [--os macos|linux] [--pkg brew|apt|dnf|pacman|zypper|apk] [--optional ctx7,deepwiki,playwright]",
	);
