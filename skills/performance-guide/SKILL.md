...
Apply these patterns when optimizing code, investigating performance issues, or reviewing for performance problems.
...
```rust
// Problem: allocating a new Vec on every iteration
for item in items {
...
async function processItems(items: Item[]) {
  for (const item of items) {
    await heavyComputation(item); // sequential, blocks other work
  }
}

// Fix: parallelize when operations are independent
async function processItems(items: Item[]) {
...
-- Fix: add index on the queried column
...
- Cache key strategy
- Eviction policy (LRU, TTL, size-based)
